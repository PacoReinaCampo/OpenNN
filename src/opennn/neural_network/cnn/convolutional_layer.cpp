...
#include "../main/pooling_layer.h"
#include "../main/probabilistic_layer.h"
#include "../pnn/perceptron_layer.h"
  ...
  for (size_t tensor_index = 0; tensor_index < size; tensor_index++) {
    ...
    for (size_t i = 0; i < next_layers_filters_number; i++) {
      for (size_t j = 0; j < next_layers_output_rows; j++) {
        if (static_cast<int>(row_index) - static_cast<int>(j * next_layers_row_stride) >= 0 && row_index - j * next_layers_row_stride < next_layers_filter_rows) {
          for (size_t k = 0; k < next_layers_output_columns; k++) {
            ...
            if (static_cast<int>(column_index) - static_cast<int>(k * next_layers_column_stride) >= 0 && column_index - k * next_layers_column_stride < next_layers_filter_columns) {
      ...
      for (size_t column_index = 0; column_index < outputs_columns_number; column_index++) {
        for (size_t window_row = 0; window_row < filter_rows_number; window_row++) {
          ...
          for (size_t window_column = 0; window_column < filter_columns_number; window_column++) {
            ...
            convolutions(row_index, column_index) += image(channel_index, row, column) * filter(channel_index, window_row, window_column);
          }
        }
      }
    }
