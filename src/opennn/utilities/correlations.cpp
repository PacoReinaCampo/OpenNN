...
double linear_correlation(const Vector<double>& x, const Vector<double>& y) {
  ...
  if (abs(s_x - 0) < numeric_limits<double>::epsilon() && abs(s_y - 0) < numeric_limits<double>::epsilon() && abs(s_xx - 0) < numeric_limits<double>::epsilon() && abs(s_yy - 0) < numeric_limits<double>::epsilon() && abs(s_xy - 0) < numeric_limits<double>::epsilon()) {
...
double logistic_error_missing_values(const double& a, const double& b, const Vector<double>& x, const Vector<double>& y) {
  const size_t n = y.size();
...
CorrelationResults karl_pearson_correlations(const Matrix<double>& x, const Matrix<double>& y) {
  ...
  if (x.get_columns_number() <= y.get_columns_number())
    k = x.get_columns_number();
  else
    k = y.get_columns_number();
