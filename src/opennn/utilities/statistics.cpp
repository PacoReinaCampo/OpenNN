...
Vector<bool> perform_normality_analysis(const Vector<double>& vector) {
  ...
  for (size_t i = 0; i < 9; i++) {
    A_significance_level = 6.32207539843126 - 17.1398870006148 * (1 - significance_level) + 38.42812675101057 * pow((1 - significance_level), 2) - 45.93241384693391 * pow((1 - significance_level), 3) + 7.88697700041829 * pow((1 - significance_level), 4) + 29.79317711037858 * pow((1 - significance_level), 5) - 18.48090137098585 * pow((1 - significance_level), 6);

    B_significance_level = 12.940399038404 - 53.458334259532 * (1 - significance_level) + 186.923866119699 * pow((1 - significance_level), 2) - 410.582178349305 * pow((1 - significance_level), 3) + 517.377862566267 * pow((1 - significance_level), 4) - 343.581476222384 * pow((1 - significance_level), 5) + 92.123451358715 * pow((1 - significance_level), 6);
...
Vector<size_t> minimal_indices_omit(const Matrix<double>& matrix, const double& value_to_omit) {
  ...
  for (size_t i = 0; i < rows_number; i++) {
    for (size_t j = 0; j < columns_number; j++) {
      if (abs(matrix(i, j) - value_to_omit) < numeric_limits<double>::min() && matrix(i, j) < minimum) {
        minimum = matrix(i, j);
        minimal_indices[0] = i;
        minimal_indices[1] = j;
      }
...
Vector<size_t> maximal_indices_omit(const Matrix<double>& matrix, const double& value_to_omit) {
  ...
  for (size_t i = 0; i < rows_number; i++) {
    for (size_t j = 0; j < columns_number; j++) {
      if (abs(matrix(i, j) - value_to_omit) < numeric_limits<double>::min() && matrix(i, j) > maximum) {
        maximum = matrix(i, j);
        maximum_indices[0] = i;
        maximum_indices[1] = j;
      }
