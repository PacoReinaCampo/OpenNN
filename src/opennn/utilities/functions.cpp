...
Matrix<double> lower_upper_bounded(const Matrix<double>& matrix, const Vector<double>& lower_bounds, const Vector<double>& upper_bounds) {
  ...
  for (size_t i = 0; i < rows_number; i++) {
    for (size_t j = 0; j < columns_number; j++) {
      if (bounded_matrix(i, j) < lower_bounds[j])
        bounded_matrix(i, j) = lower_bounds[j];
      else if (bounded_matrix(i, j) > upper_bounds[j])
        bounded_matrix(i, j) = upper_bounds[j];
    }
  }
...
Tensor<double> lower_upper_bounded(const Tensor<double>& tensor, const Vector<double>& lower_bounds, const Vector<double>& upper_bounds) {
  ...
  for (size_t i = 0; i < rows_number; i++) {
    for (size_t j = 0; j < columns_number; j++) {
      if (bounded_tensor(i, j) < lower_bounds[j])
        bounded_tensor(i, j) = lower_bounds[j];
      else if (bounded_tensor(i, j) > upper_bounds[j])
        bounded_tensor(i, j) = upper_bounds[j];
    }
  }
